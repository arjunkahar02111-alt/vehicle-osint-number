...
function assertDb(result: { error?: { message?: string } | null }) {
  if (result.error) throw new Error(result.error.message || "Backend query failed");
}
...
async function loadBlockSet(): Promise<Set<string>> {
  ...
  const res = await supabaseAdmin.from("blocked_ips" as never).select("ip");
  assertDb(res);
  const { data } = res;
...
export async function resetVisitorCredits(ip: string): Promise<void> {
  const res = await supabaseAdmin.from("admin_settings" as never).delete().eq("key", creditKey(ip));
  assertDb(res);
}
...
export async function unblockIP(ip: string): Promise<void> {
  const res = await supabaseAdmin.from("blocked_ips" as never).delete().eq("ip", ip);
  assertDb(res);
  invalidateBlocks();
}
...
export async function recordLog(entry: Omit<LookupLog, "id" | "ts">): Promise<void> {
  const res = await supabaseAdmin.from("lookup_logs" as never).insert({
    query: entry.query,
    status: entry.status,
    ip: entry.ip,
...
export async function statsSummary(): Promise<StatsSummary> {
  ...
  [totalRes, blockedRes, last24Res, lastHourRes].forEach(assertDb);
